...
func (ts *typeScriptLang) addProjectRule(cfg *JsGazelleConfig, tsconfigRel string, tsconfig *typescript.TsConfig, args language.GenerateArgs, group *TargetGroup, targetName string, sourceFiles, genFiles, dataFiles []string, result *language.GenerateResult) (*rule.Rule, error) {
	...
	if cfg.GetTsConfigGenerationEnabled(group.name) {
		// Reflect ts_project(tsconfig) and related attributes from the tsconfig
		// unless they have been explicitly opted out via js_tsconfig_ignore.
		// For non-ts_project rule kinds the kind-based default in
		// IsTsConfigIgnored treats every reflected attr as ignored, preserving
		// the historical "strip all" behavior until the user opts in with a
		// NONE sentinel on the group.

		// First strip ignored attrs from the existing rule so that merge
		// doesn't carry them back. This replaces the previous unconditional
		// "if ruleKind != TsProjectKind" strip block — that block stripped
		// every reflected attr from existing; we now strip per-attr based on
		// IsTsConfigIgnored, which honors NONE-driven un-ignores.
		if existing != nil {
			for _, attr := range tsProjectReflectedConfigAttributes {
				if cfg.IsTsConfigIgnored(group.name, ruleKind, attr) {
					existing.DelAttr(attr)
				}
			}
		}

		if !cfg.IsTsConfigIgnored(group.name, ruleKind, "tsconfig") {
			if tsconfig != nil {
		...
		if !cfg.IsTsConfigIgnored(group.name, ruleKind, "allow_js") {
		...
		if !cfg.IsTsConfigIgnored(group.name, ruleKind, "composite") {
		...
		if !cfg.IsTsConfigIgnored(group.name, ruleKind, "declaration") {
		...
		if !cfg.IsTsConfigIgnored(group.name, ruleKind, "declaration_map") {
		...
		if !cfg.IsTsConfigIgnored(group.name, ruleKind, "emit_declaration_only") {
		...
		if !cfg.IsTsConfigIgnored(group.name, ruleKind, "source_map") {
		...
		if !cfg.IsTsConfigIgnored(group.name, ruleKind, "incremental") {
		...
		if !cfg.IsTsConfigIgnored(group.name, ruleKind, "ts_build_info_file") {
		...
		if !cfg.IsTsConfigIgnored(group.name, ruleKind, "no_emit") {
		...
		if !cfg.IsTsConfigIgnored(group.name, ruleKind, "resolve_json_module") {
		...
		if !cfg.IsTsConfigIgnored(group.name, ruleKind, "preserve_jsx") {
		...
		if !cfg.IsTsConfigIgnored(group.name, ruleKind, "out_dir") {
		...
		if !cfg.IsTsConfigIgnored(group.name, ruleKind, "declaration_dir") {
		...
		if !cfg.IsTsConfigIgnored(group.name, ruleKind, "root_dir") {
			pkgRelRootDir := "."
			if tsconfig != nil {
		...
		if existing != nil {
			for _, attr := range tsProjectReflectedConfigAttributes {
				if !cfg.IsTsConfigIgnored(group.name, ruleKind, attr) && existing.Attr(attr) != nil {
